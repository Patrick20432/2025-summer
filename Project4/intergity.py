...
def sm3_len_ext_attack(orig_hash, orig_len, append_msg):
    ...
    append_padded = sm3_pad(append_msg)
    V = state[:]
...
class MerkleTree:
    def __init__(self, leaves):
        self.leaves = leaves  # 原顺序存储
        self.leaf_hashes = [bytes.fromhex(sm3_hash(l)) for l in self.leaves]
        self.levels = []
        self.build_tree()
    ...
    def get_non_inclusion_proof(self, element):
        idx = 0
        while idx < len(self.sorted_leaves) and self.sorted_leaves[idx] < element:
            idx += 1
        prev_leaf = self.sorted_leaves[idx - 1] if idx > 0 else None
        next_leaf = self.sorted_leaves[idx] if idx < len(self.sorted_leaves) else None
        proof_prev = None
        proof_next = None
        idx_prev = None
        idx_next = None
        if prev_leaf:
            idx_prev = self.leaves.index(prev_leaf)
            proof_prev = self.get_proof(idx_prev)
        if next_leaf:
            idx_next = self.leaves.index(next_leaf)
            proof_next = self.get_proof(idx_next)
        return (prev_leaf, proof_prev, idx_prev), (next_leaf, proof_next, idx_next)

    @staticmethod
    def verify_non_inclusion_proof(element, prev_proof, next_proof, root):
        prev_leaf, prev_pf, prev_idx = prev_proof
        next_leaf, next_pf, next_idx = next_proof
        ok_prev = True
        ok_next = True
        if prev_leaf is not None:
            ok_prev = MerkleTree.verify_proof(prev_leaf, prev_pf, root, prev_idx)
        if next_leaf is not None:
            ok_next = MerkleTree.verify_proof(next_leaf, next_pf, root, next_idx)
        between = (prev_leaf is None or prev_leaf < element) and (next_leaf is None or element < next_leaf)
        return ok_prev and ok_next and between
...
if __name__ == "__main__":
    # 基础 SM3
    print("[基础 SM3]", sm3_hash(b"abc"))

    # 优化版 SM3
    print("[优化 SM3]", sm3_hash_fast(b"abc"))

    # 长度扩展攻击
    orig = b"test"
    h = sm3_hash(orig)
    new_hash, pad = sm3_len_ext_attack(h, len(orig), b"admin=true")
    print("[长度扩展攻击] 原哈希:", h)
    print("[长度扩展攻击] 新哈希:", new_hash)
    ...
    t2 = time.time()
    print(f"[Merkle Tree] {N} 叶子构建完成, 耗时 {t2 - t1:.2f} 秒")
    root = mt.get_root()
    ...
    print("[存在性验证结果]", MerkleTree.verify_proof(leaves[idx], proof, root, idx))

    # 不存在性证明
    elem = b"no_such_leaf"
    prev_pf, next_pf = mt.get_non_inclusion_proof(elem)
    ok = MerkleTree.verify_non_inclusion_proof(elem, prev_pf, next_pf, root)
    print("[不存在性验证结果]", ok)

    # 性能测试 10 万叶子（可选）
    N_big = 10**5
    leaves_big = [f"leaf{i}".encode() for i in range(N_big)]
    t1 = time.time()
    MerkleTree(leaves_big)
    t2 = time.time()
    print(f"[性能测试] {N_big} 叶子构建耗时: {t2 - t1:.2f} 秒")