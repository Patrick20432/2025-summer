...
通过本次实验，我们成功实现并验证了一个基于 DCT 域的数字水印嵌入和提取系统。实验结果表明，在你当前的参数设置下，该水印算法的鲁棒性普遍较低，所有测试攻击下的 NCC 值均在 0.09 到 0.33 之间，这说明水印很容易被各种图像处理操作破坏。其中，JPEG 压缩和几何攻击（特别是裁剪和旋转）对水印的影响尤为显著。
